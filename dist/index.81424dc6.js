/*eslint-disable*/ // creat Count Time Oppining✅
function cowndown() {
    ...
    if (textsecand < 10) document.querySelector(".condown__secand").innerText = `0${textsecand}`;
    else document.querySelector(".condown__secand").innerText = textsecand;
}
...
let skills = skillsArray.map((item)=>{
    return `  <li data-type="${item.type}" data-name="${item.name}"class="skills_items">
            <div>
              <h2 class="skills__title">${item.name}</h2>
              <span class="skills__years">${newDate - item.experience} years</span>
            </div>
            ${item.class}
          </li>
  `;
...
/* Boens */ document.querySelectorAll(".skills__button").forEach((element)=>{
...
//# sourceMappingURL=index.81424dc6.js.map
